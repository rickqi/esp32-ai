...
| **v3 中文** | **zh6** | **15.8M** | 22.35 | Qwen3-0.6B 数据蒸馏（词表 7,563） |
| **v3 中文** | **zh6-distill** | **15.8M** | 9.2 | 数据蒸馏 SFT |
| **v3 中文** | **zh6-raft** | **15.8M** | **2.7** | RAFT 证据复述（model.bin 8.92MB） |

### V3 蒸馏版本（2026-08-02 完成）

```
目标: 单台 ESP32 24M 蒸馏（实际 15.8M，8.92MB int4）
蒸馏源: Qwen3-0.6B（数据蒸馏；logits KL 因词表不匹配不可行）
词表: 7,563（v2 为 6,594，更宽）
流程: zh6 预训练 → 数据蒸馏 SFT → RAFT → 量化导出
独立: chinese_v3/ + data_v3/ + runs_v3/ + firmware/model_v3/
