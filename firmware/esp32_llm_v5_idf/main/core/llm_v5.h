...
// output-head matvec.
//
// V5 (esp32_llm_zh_v5): fork of shared llm.h for external MiniMind PLE models
// (H1/H2). Adds per-head q_norm/k_norm (MiniMind RMSNorm on q/k before RoPE)
// and expects qkv as a pre-concatenated [3D,D] tensor (converter merges
// MiniMind's separate q/k/v).  Matches MiniMind model_minimind.py op-for-op:
// split-half RoPE, erf-GELU, SiLU-SwiGLU, RMSNorm(x*rsqrt(mean(x^2)+eps)),
// tied input/output embedding, and the PLE input
// (RMSNorm(proj(x)/sqrt(D)) + table[tok]*sqrt(P)) / sqrt(2)
...
typedef struct {
  ...
  const float *q_norm[32];    // [head_dim] per-head RMSNorm on q (MiniMind V5)
  const float *k_norm[32];    // [head_dim] per-head RMSNorm on k (MiniMind V5)
  QT qkv[32];             // [3D, D]
  QT attn_proj[32];       // [D, D]
...
static int llm_load(const uint8_t *base, Model *m) {
  ...
  int Dh = D / m->c.n_heads;   // head_dim (V5 q_norm/k_norm size)
...
p = bind_f(p, &m->q_norm[i], Dh);     // V5: per-head q RMSNorm (head_dim)
p = bind_f(p, &m->k_norm[i], Dh);     // V5: per-head k RMSNorm (head_dim)
p = bind_q(p, &m->qkv[i], 3 * D, D);  // V5: q/k/v pre-concatenated by converter
...
static void llm_forward(Model *m, int token, int pos, Scratch *s) {
  ...
  for (int l = 0; l < L; l++) {
    ...
    // V5: per-head RMSNorm on q/k (MiniMind q_norm/k_norm), before RoPE
