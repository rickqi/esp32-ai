...
```
┌────────────────────────────────────────────┐  ← 2px 像素边框
█  E E S S P P 3 3 - -   P P L E   L L M  █  ← 标题栏（2x 大字体,白字黑底）
├────────────────────────────────────────────┤  ← 分隔线
  > Rick Zhang Say                               ← Prompt 输入区
├────────────────────────────────────────────┤  ← 分隔线
  was very sad and started to cry...            ← 推理输出区（可滚动）
  ...                                           ← 不越界,在标题和底栏之间
├────────────────────────────────────────────┤  ← 分隔线
█ 10.1t/s  099ms  28.9M  S3-N16R8  07/30 17:03 █  ← 固定底栏（反白,不换行）
└────────────────────────────────────────────┘  ← 2px 像素边框
```
...
| 日期时间 | `%m/%d %H:%M` | `07/30 17:03` | 右对齐 | 66px (11ch) | PCF85063 RTC 或编译时间回退 |
