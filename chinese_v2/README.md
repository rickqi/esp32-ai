...
## 三、量化导出评估（结论: 需要, 用 group=32）

| 检查项 | 值 | 结论 |
|---|---|---|
| 模型格式 | fp32 PyTorch 权重 | **必须 4-bit PTQ 量化** |
| PSRAM 需求 | 3.73M / 8M | ✅ 设备可容纳 |
| Flash 需求 | ~7.10M / 14.5M | ✅ 设备可容纳 |
| 词表差异 | 6,594 ≠ v1 5,904 | **需独立 vocab.h + model.bin** |

### ⚠️ 量化敏感性（实测，2026-07-31）

| 配置 | 生成效果 | 结论 |
|---|---|---|
| **4-bit group=128** | ❌ 崩溃（">>>>>"） | 过粗，SFT 模型敏感 |
| **4-bit group=32** | ✅ 有医学内容（"发热、恶心、呕吐、食欲不振"） | **可用** |
| 4-bit group=16 | ❌ 崩 | 过细过拟合 |
| **8-bit group=64** | ✅ 完全保真（≈FP32） | 最优但 model.bin ~14MB |

**导出配置**：`chinese/export.py` GROUP 已改为 **32**（SFT 模型 4-bit 敏感，128 崩溃）。
