...
Coverage target: 完整 GB2312 (7445 chars) + model vocab CJK chars, rendered as
14x14 1bpp cells (25 bytes each). The source font covers 98.71% of GB2312
(measured); the remaining 87 chars fall back to □ in the firmware.

Output: firmware/esp32_llm_zh/cjk_font.h  (same CJK_CP/CJK_OFF/CJK_BLOB format)
"""
...
CBIN = Path(r"D:\codes\ESP32-S3-RLCD\docs\examples\ESP32-S3-RLCD-4.2-GitHub\02_Example\XiaoZhi\XiaoZhiCode_V2.1.0\managed_components\78__xiaozhi-fonts\cbin\font_noto_qwen_14_1.bin")
OUT = PROJECT / "firmware" / "esp32_llm_zh" / "cjk_font.h"
TOKENIZER = PROJECT / "data_chinese" / "tokenizer.json"
...
# ---- 2. target char set: full GB2312 + model vocab CJK ----------------------
def gb2312_chars():
    out = []
    for q in range(1, 88):
        for w in range(1, 95):
...
with open(OUT, "w", encoding="utf-8") as f:
    ...
    f.write("// Source: XiaoZhi 78/xiaozhi-fonts font_noto_qwen_14_1.bin (GB2312 ~98.7%)\n")
...
print(f"wrote {OUT}")
print(f"glyphs={len(cps)} blob={len(blob)/1024:.0f}KB  (+CP {len(cps)*2/1024:.0f}KB +OFF {len(offs)*2/1024:.0f}KB)")
print(f"GB2312 coverage: {sum(1 for cp,_ in found if 0x4E00<=cp<=0x9FFF)}/6763 "
      f"({100*sum(1 for cp,_ in found if 0x4E00<=cp<=0x9FFF)/6763:.1f}%)")
